...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'All':
        ...
        filtered_df["Counts"] = np.zeros(len(filtered_df))
        filtered_df=filtered_df.groupby(['Launch Site','class']).count()
        filtered_df.reset_index(level=['Launch Site','class'], inplace=True)
        filtered_df=filtered_df[['Launch Site','class', 'Counts']]
...
def scatter_chart(entered_site,payload): 
    #Filter using the max and min range of payload 
    filtered_df = spacex_df[spacex_df['Payload Mass (kg)'].between(payload[0],payload[1])]
    filtered_df[(filtered_df['Payload Mass (kg)']>payload[0]) & (filtered_df['Payload Mass (kg)']< payload[1])]
    if entered_site == 'ALL': 
